...
class PositionEmbedding2D(nn.Module):
    def __init__(self, max_source_positions: int, embed_dim: int):
        super().__init__()
        max_height = tree_height(max_source_positions) + 1

        self.col_emb = nn.Embedding(max_source_positions, embed_dim)
...
class TreeformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in TreeformerModel forward() function)
...
class TreeformerAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = TreeformerSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = TreeformerSelfOutput(config)
...
class TreeformerLayer(nn.Module):
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = TreeformerAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = TreeformerAttention(config, position_embedding_type="absolute")
        self.intermediate = TreeformerIntermediate(config)
        self.output = TreeformerOutput(config)
...
class TreeformerEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([TreeformerLayer(config) for _ in range(config.num_hidden_layers)])
...
class TreeformerPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, TreeformerEncoder):
...
class TreeformerModel(TreeformerPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = TreeformerEmbeddings(config)
        self.encoder = TreeformerEncoder(config)

        self.pooler = TreeformerPooler(config) if add_pooling_layer else None
...
class TreeformerForCausalLM(TreeformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `TreeformerLMHeadModel` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, TreeformerForCausalLM, AutoConfig
        >>> import torch

        >>> tokenizer = AutoTokenizer.from_pretrained("offendo/treeformer-base")
        >>> config = AutoConfig.from_pretrained("offendo/treeformer-base")
        >>> config.is_decoder = True
        >>> model = TreeformerForCausalLM.from_pretrained("offendo/treeformer-base", config=config)
...
class TreeformerForMaskedLM(TreeformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TreeformerForMaskedLM` make sure `config.is_decoder=False` for "
...
class TreeformerForSequenceClassification(TreeformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.treeformer = TreeformerModel(config, add_pooling_layer=False)
        self.classifier = TreeformerClassificationHead(config)
...
class TreeformerForTokenClassification(TreeformerPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        TREEFORMER_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length")
    )
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    @add_start_docstrings_to_model_forward(TREEFORMER_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
        checkpoint="Jean-Baptiste/treeformer-large-ner-english",
...
class TreeformerForQuestionAnswering(TreeformerPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(TREEFORMER_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
        checkpoint="deepset/offendo/treeformer-base-squad2",
