...
from transformers import TreeformerConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers.models.treeformer.modeling_tf_treeformer import (
...
class TFTreeformerModelTester:
    ...
    def create_and_check_model(
        ...
        model = TFTreeformerModel(config=config)
    ...
    def create_and_check_causal_lm_model(
        ...
        model = TFTreeformerForCausalLM(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_causal_lm_model_past(
        ...
        # special to `TreeformerEmbeddings` in `Treeformer`:
        #   - its `padding_idx` and its effect on `position_ids`
        #     (TFTreeformerEmbeddings.create_position_ids_from_input_ids)
        #   - `1` here is `TFTreeformerEmbeddings.padding_idx`
    ...
    def create_and_check_for_masked_lm(
        ...
        model = TFTreeformerForMaskedLM(config=config)
    ...
    def create_and_check_for_token_classification(
        ...
        model = TFTreeformerForTokenClassification(config=config)
        inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        result = model(inputs)
    ...
    def create_and_check_for_question_answering(
        ...
        model = TFTreeformerForQuestionAnswering(config=config)
        inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        result = model(inputs)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = TFTreeformerForMultipleChoice(config=config)
...
@require_tf
class TFTreeformerModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            TFTreeformerModel,
            TFTreeformerForCausalLM,
    ...
    def setUp(self):
        self.model_tester = TFTreeformerModelTester(self)
    ...
    def test_model_from_pretrained(self):
        for model_name in TF_TREEFORMER_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = TFTreeformerModel.from_pretrained(model_name)
...
@require_tokenizers
class TFTreeformerModelIntegrationTest(unittest.TestCase):
    @slow
    def test_inference_masked_lm(self):
        model = TFTreeformerForMaskedLM.from_pretrained("offendo/treeformer-base")
    ...
    def test_inference_no_head(self):
        model = TFTreeformerModel.from_pretrained("offendo/treeformer-base")
    ...
    def test_inference_classification_head(self):
        model = TFTreeformerForSequenceClassification.from_pretrained("treeformer-large-mnli")
